...
import type { Cliente, Vehiculo, Refaccion, CompatibilidadVehiculo, Proveedor } from '../lib/types';
import { CATEGORIAS, UNIDADES } from '../lib/constants';
import { fmt, labelVehiculo } from '../lib/utils';
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            ...
            <div>
              ...
              <Input type="number" placeholder="0.00" min="0.01" step="0.01"
                value={form.precioCompra || ''} onChange={e => setForm(f => ({ ...f, precioCompra: Number(e.target.value) }))} required />
            </div>
            <div>
                ...
                <div className="flex items-center gap-2">
                  <div className="flex-1">
                    <Input type="text" placeholder="Marca (ej. Ford, Isuzu, VW...)"
                      value={grupo.marca} onChange={e => updateCompatMarca(gi, e.target.value)} />
                  </div>
                ...
                {grupo.marca && (
                  <>
                    <div className="flex flex-wrap gap-1.5">
                      ...
                      {grupo.modelos.length === 0 && <span className="text-xs text-amber-600">Agrega al menos un modelo</span>}
                    </div>
                    ...
                    <>
                      <tr key={r.id} className={i % 2 === 0 ? 'bg-white' : 'bg-slate-50'}>
                        ...
                        <td className="px-4 py-3">
                          ...
                          {vLabel && (
                            <div className="mt-0.5 flex items-center gap-1">
                              <span className="text-xs bg-violet-100 text-violet-700 font-semibold px-2 py-0.5 rounded-full">🎯 {vLabel}</span>
                            </div>
                          )}
                          {proveedorNombre(r.proveedorId) && (
                            <div className="mt-0.5">
                              <span className="text-xs bg-slate-100 text-slate-600 font-medium px-2 py-0.5 rounded-full">🏪 {proveedorNombre(r.proveedorId)}</span>
                            </div>
                          )}
                        ...
                        <td className="px-4 py-3 text-center">
                          <Btn size="sm" variant={isExp ? 'ghost' : 'success'} onClick={() => setExpandido(isExp ? null : r.id)}>
                            ...
                            <div className="flex items-center gap-3 flex-wrap">
                              ...
                              <div className="flex items-center gap-2">
                                ...
                                <Btn size="sm" variant="success"
                                  ...
                                  onClick={() => handleRecibir(r.id)}>✓ Confirmar</Btn>
                              </div>
