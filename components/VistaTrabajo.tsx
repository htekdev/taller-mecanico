...
import type { Cliente, Vehiculo, Refaccion, Trabajo, TrabajoRefaccion, ManoDeObraItem, PricingIntel } from '../lib/types';
import { fmt, labelVehiculo, getPricingIntel, isPartCompatibleWithVehiculo } from '../lib/utils';
import { Label, Input, Select, Btn, SectionTitle, EmptyRow } from '../components/ui';
...
export default function VistaTrabajo({
  ...
  const agregarParte = () => {
    ...
    setPartesSeleccionadas(prev => {
      ...
      return [...prev, {
        ...
        cantidad: pickerCantidad, precioCompra: ref.precioCompra,
        precioVenta: pVenta,
        subtotal: pickerCantidad * pVenta,
        costoTotal: pickerCantidad * ref.precioCompra,
      }];
    });
    setPickerRefId(''); setPickerCantidad(1); setPickerPrecioVenta(0);
  };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setForm(emptyForm);
    setLaborItems([]); setLaborConcepto(''); setLaborPrecio(0);
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="flex gap-2 flex-wrap items-end">
                <div className="flex-1 min-w-48">
                  ...
                  <Input type="text" placeholder="Ej. Arreglo de frenos, engrase de pernos..."
                    value={laborConcepto} onChange={e => setLaborConcepto(e.target.value)}
                    onKeyDown={e => { if (e.key === 'Enter') { e.preventDefault(); agregarLabor(); } }} />
                </div>
                <div className="w-36">
                  <Label>Precio ($)</Label>
                  <Input type="number" placeholder="0.00" min="0.01" step="0.01"
                    value={laborPrecio || ''} onChange={e => setLaborPrecio(Number(e.target.value))} />
                </div>
          ...
          <div className="border border-slate-200 rounded-xl bg-white overflow-hidden">
            <div className="px-4 py-3 bg-slate-800 flex items-center justify-between">
              ...
              {inventario.length === 0 && (
                <button type="button" onClick={onIrAInventario} className="text-xs text-indigo-300 hover:text-white underline font-medium">
                  Ir a Inventario →
                </button>
              )}
            </div>
            ...
            {inventario.length === 0 ? (
              <div className="px-4 py-6 text-center text-slate-400 text-sm">
                ...
                <button type="button" onClick={onIrAInventario} className="mt-2 text-indigo-600 font-semibold hover:underline">
              ...
              <div className="p-4 space-y-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 items-end">
                    ...
                    <Select value={pickerRefId} onChange={e => handlePickerRefChange(e.target.value)}>
                      ...
                      {partesParaEstaUnidad.length > 0 && (
                        <optgroup label="🎯 Compradas para esta unidad">
                          {partesParaEstaUnidad.map(r => <option key={r.id} value={r.id}>{r.nombre}{r.codigo ? ` (${r.codigo})` : ''} — {r.stock} {r.unidad} en stock</option>)}
                        </optgroup>
                      )}
                      {partesCompatibles.length > 0 && vehiculoDelTrabajo && (
                        <optgroup label={`✅ Compatible con ${vehiculoDelTrabajo.marca} ${vehiculoDelTrabajo.modelo}`}>
                          {partesCompatibles.map(r => <option key={r.id} value={r.id}>{r.nombre}{r.codigo ? ` (${r.codigo})` : ''} — {r.stock} {r.unidad} en stock</option>)}
                        </optgroup>
                      )}
                      {partesUniversales.length > 0 && (
                        <optgroup label="🌐 Universales (todos los vehículos)">
                          {partesUniversales.map(r => <option key={r.id} value={r.id}>{r.nombre}{r.codigo ? ` (${r.codigo})` : ''} — {r.stock} {r.unidad} en stock</option>)}
                        </optgroup>
                      )}
                      {!form.vehiculoId && inventario.map(r => (
                        <option key={r.id} value={r.id}>{r.nombre}{r.codigo ? ` (${r.codigo})` : ''} — {r.stock} {r.unidad} en stock</option>
                      ))}
                    </Select>
                  </div>
                  <div>
                    <Label>Cantidad</Label>
                    <Input type="number" min="1" step="1" placeholder="1" value={pickerCantidad || ''}
                  ...
                  <div>
                    ...
                    <Input type="number" min="0" step="0.01" placeholder="0.00"
                      ...
                      className={intel?.clientLastSale && pickerPrecioVenta > 0 && pickerPrecioVenta < intel.clientLastSale.precio ? '!border-amber-400 !ring-amber-400' : ''} />
                  </div>
                </div>
                ...
                {pickerRef && intel && (
                  <div className="bg-white border border-slate-200 rounded-xl p-4 space-y-3 text-sm">
                    <div className="flex items-center flex-wrap gap-2">
                      ...
                      {intel.markups.map(m => (
                        <button key={m.pct} type="button" onClick={() => setPickerPrecioVenta(m.price)}
                          className={`px-2.5 py-1 rounded-lg text-xs font-semibold border transition-all ${
                            pickerPrecioVenta === m.price ? 'bg-indigo-600 text-white border-indigo-600' : 'bg-white text-indigo-600 border-indigo-200 hover:bg-indigo-50'
                          }`}>
                          {m.pct}% margen ${fmt(m.price)}
                        </button>
                      ))}
                      {pickerRef.stock < pickerCantidad && (
                        <span className="text-xs font-semibold text-amber-600 ml-auto">⚠ solo {pickerRef.stock} en stock</span>
                      )}
                    </div>
                        ...
                        <div className={`flex items-start justify-between gap-3 rounded-lg px-3 py-2.5 border ${isLower ? 'bg-amber-50 border-amber-200' : 'bg-indigo-50 border-indigo-200'}`}>
                          <div className="text-xs">
                            <span className={`font-bold ${isLower ? 'text-amber-700' : 'text-indigo-700'}`}>
                              {isLower ? '⚠️' : '📋'}{' '}
                              {isLower ? `Estás cobrando menos que antes ($${fmt(pickerPrecioVenta)} vs $${fmt(intel.clientLastSale!.precio)})`
                                : isSame ? `Mismo precio que el máximo cobrado ($${fmt(intel.clientLastSale!.precio)})`
                          ...
                          {!isSame && (
                            <button type="button" onClick={() => setPickerPrecioVenta(intel.clientLastSale!.precio)}
                              className={`text-xs font-bold whitespace-nowrap px-2.5 py-1 rounded-lg border transition-all ${
                                isLower ? 'bg-amber-600 text-white border-amber-600 hover:bg-amber-700' : 'bg-indigo-600 text-white border-indigo-600 hover:bg-indigo-700'
                              }`}>
                      ...
                      <div className="text-xs text-slate-400 flex items-center gap-1">
                        ...
                        <span>Otros clientes pagaron:{' '}
                          <strong className="text-slate-600">
                            {intel.otherMin === intel.otherMax ? `$${fmt(intel.otherMin!)}` : `$${fmt(intel.otherMin!)} – $${fmt(intel.otherMax!)}`}
                          </strong>
                        </span>
                      </div>
                    )}
                ...
                <div className="flex justify-between items-center gap-3 flex-wrap">
                  ...
                  <div className={!pickerRef ? 'ml-auto' : ''}>
                    <Btn variant="primary" disabled={!pickerRefId || pickerCantidad <= 0} onClick={agregarParte}>+ Agregar pieza</Btn>
                  </div>
                </div>
                          ...
                          return (
                            <tr key={p.refaccionId} className="bg-white">
                              <td className="px-3 py-2 text-slate-800 font-medium">
                                {p.nombre}{p.codigo && <span className="ml-1.5 text-xs font-mono text-slate-400">{p.codigo}</span>}
                              </td>
                              ...
                              <td className="px-3 py-2 text-center"><Btn size="sm" variant="danger" onClick={() => removerParte(p.refaccionId)}>✕</Btn></td>
                            </tr>
                          );
                        })}
                      </tbody>
