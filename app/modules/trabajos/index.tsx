...
export function VistaTrabajo({
  ...
  const emptyForm = {    clienteId: '', vehiculoId: '',
  ...
  const [filtroLista, setFiltroLista] = useState<'todos'|'pendientes'|'con_factura'|'sin_factura'>('todos');
    ...
    <div>
      ...
      <div className={`border rounded-xl p-5 mb-8 ${editandoId ? 'bg-amber-50 border-amber-200' : 'bg-slate-50 border-slate-200'}`}>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <Btn type="submit" variant="primary" fullWidth
            ...
            {editandoId ? '💾 Guardar Cambios' : '✓ Registrar Trabajo'}
          </Btn>
        </form>
      </div>
      ...
      <div>
        ...
        <div className="mb-4 space-y-3">
          <Input
            type="text"
            placeholder="🔍 Buscar trabajo por cliente, vehículo o descripción..."
            value={busqueda}
          ...
          <div className="flex gap-2 flex-wrap">
            {([
              ...
              <Btn
                key={key}
                size="sm"
                variant={filtroLista === key ? 'primary' : 'ghost'}
                onClick={() => setFiltroLista(key)}
              >
                {label}
              </Btn>
            ))}
        ...
        {(() => {
          ...
          // Apply status filter
          if (filtroLista === 'pendientes') {
            filtrados = filtrados.filter(t => t.estado === 'pendiente');
          } else if (filtroLista === 'con_factura') {
            filtrados = filtrados.filter(t => t.estadoFacturacion === 'facturado');
          } else if (filtroLista === 'sin_factura') {
            filtrados = filtrados.filter(t => t.estadoFacturacion !== 'facturado');
          }
          ...
          if (q) {
            filtrados = filtrados.filter(t => {
              ...
              const vehiculoStr = vehiculo
                ? [vehiculo.anio, vehiculo.marca, vehiculo.modelo, vehiculo.placa].filter(Boolean).join(' ').toLowerCase()
                : '';
              return (
                    ...
                    <td className="px-4 py-3 text-center">
                      {trabajo.estado === 'pendiente' && (
                        <button type="button"
                          ...
                          className="text-xs font-semibold text-indigo-600 hover:text-indigo-800 bg-indigo-50 hover:bg-indigo-100 px-2 py-1 rounded-lg transition-colors whitespace-nowrap">
                          ✏️ Editar
                        </button>
                      )}
                    </td>
