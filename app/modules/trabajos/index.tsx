...
async function generarComprobantePago(
  ...
  if (trabajo.manoDeObraItems?.length > 0) {
    ...
    trabajo.manoDeObraItems.forEach((item, i) => {
      ...
      doc.text(`$${fmtPesoA(itemTotal)}`, ml + cw, y + 4, { align: 'right' });
    ...
    doc.text(`$${fmtPesoA(trabajo.manoDeObra)}`, ml + cw, y + 4, { align: 'right' });
    y += 8;
  }
  ...
  if (trabajo.manoDeObra > 0) sumLine('Mano de obra:', `$${fmtPesoA(trabajo.manoDeObra)}`);
  if (trabajo.refacciones > 0) sumLine('Refacciones:', `$${fmtPesoA(trabajo.refacciones)}`);
  if (trabajo.iva > 0) sumLine('IVA (16%):', `$${fmtPesoA(trabajo.iva)}`);
  ...
  doc.text(`$${fmtPesoA(trabajo.total)}`, ml + cw, y, { align: 'right' });
  y += 12;
  ...
  if (trabajo.pagos?.length > 0) {
    ...
    trabajo.pagos.forEach((pago, i) => {
      ...
      doc.text(`$${fmtPesoA(pago.monto)}`, ml + cw, y + 4, { align: 'right' });
