...
// ─── PDF Generator — Reporte Cuentas por Cobrar ───────────────────────────────
...
async function generarPDFReporte(
  ...
  const pw = 215.9, ml = 14, mr = 14, cw = pw - ml - mr;

  // ── Logo ──
  const logoB64 = await loadLogoBase64();
  let y = 14;
  if (logoB64) {
    doc.addImage(logoB64, 'JPEG', ml, y, 18, 18);
  }

  // ── Header ──
  const hx = ml + 22;
  doc.setFillColor(30, 41, 59); doc.rect(hx, y, cw - 22, 18, 'F');
  doc.setFont('helvetica', 'bold'); doc.setFontSize(11); doc.setTextColor(255, 255, 255);
  doc.text('MICRO DIESEL DE MERIDA', hx + 3, y + 6);
  doc.setFont('helvetica', 'normal'); doc.setFontSize(7.5); doc.setTextColor(200, 210, 225);
  doc.text('Héctor Armando Rocha Sepúlveda', hx + 3, y + 11);
  doc.text('Circuito Colonias No. 752 x 64j y 64k, Col. Castilla Cámara, CP 97278, Mérida, Yucatán', hx + 3, y + 15);
  doc.text('Tel (999) 317.22.46  ·  Cel. 999 3597970', hx + 3, y + 18.5);
  doc.setTextColor(0, 0, 0);
  y += 24;

  // ── Document title ──
  const fechaHoy = new Date().toLocaleDateString('es-MX', { day: '2-digit', month: 'long', year: 'numeric' });
  doc.setFillColor(71, 85, 105); doc.rect(ml, y, cw, 8, 'F');
  doc.setFont('helvetica', 'bold'); doc.setFontSize(10); doc.setTextColor(255, 255, 255);
  doc.text('ESTADO DE CUENTA — CUENTAS POR COBRAR', ml + 3, y + 5.5);
  doc.setFontSize(8); doc.setFont('helvetica', 'normal');
  doc.text(fechaHoy, ml + cw - 3, y + 5.5, { align: 'right' });
  doc.setTextColor(0, 0, 0);
  y += 12;

  // ── Client info box ──
  doc.setFillColor(241, 245, 249); doc.rect(ml, y, cw, 12, 'F');
  doc.setDrawColor(203, 213, 225); doc.rect(ml, y, cw, 12, 'S');
  doc.setFont('helvetica', 'bold'); doc.setFontSize(9); doc.setTextColor(30, 41, 59);
  doc.text(cliente.nombre.toUpperCase(), ml + 3, y + 5);
  doc.setFont('helvetica', 'normal'); doc.setFontSize(7.5); doc.setTextColor(100, 116, 139);
  const contactLine: string[] = [];
  if (cliente.telefono) contactLine.push(`Tel: ${cliente.telefono}`);
  if ((cliente as Cliente & { email?: string }).email) contactLine.push(`Email: ${(cliente as Cliente & { email?: string }).email}`);
  if (contactLine.length) doc.text(contactLine.join('  ·  '), ml + 3, y + 10);
  doc.setTextColor(0, 0, 0);
  y += 16;

  // ── Table drawing helper ──
  const rh = 6.5;
  const drawTableHeader = (headers: { label: string; w: number; align?: 'left' | 'right' | 'center' }[]) => {
    doc.setFillColor(30, 64, 175); doc.rect(ml, y, cw, rh, 'F');
    doc.setFont('helvetica', 'bold'); doc.setFontSize(7); doc.setTextColor(255, 255, 255);
    let cx = ml;
    headers.forEach(h => {
      const tx = h.align === 'right' ? cx + h.w - 2 : h.align === 'center' ? cx + h.w / 2 : cx + 2;
      doc.text(h.label, tx, y + 4.3, { align: h.align ?? 'left' });
      cx += h.w;
    });
    doc.setTextColor(0, 0, 0);
    y += rh;
  };
  const drawTableRow = (cells: { text: string; w: number; align?: 'left' | 'right' | 'center'; bold?: boolean }[], idx: number) => {
    ...
    cells.forEach(c => {
      ...
      doc.text(txt, tx, y + 4.3, { align: c.align ?? 'left' });
      if (c.bold) doc.setFont('helvetica', 'normal');
      cx += c.w;
    });
    doc.setDrawColor(226, 232, 240); doc.line(ml, y + rh, ml + cw, y + rh);
    y += rh;
  };

  // ── Facturas pendientes ──
  const facsPend = facturas.filter(f => getEstadoPagoFactura(f) !== 'pagado');
  if (facsPend.length > 0) {
    // Section label
    doc.setFillColor(239, 246, 255); doc.rect(ml, y, cw, 6, 'F');
    doc.setFont('helvetica', 'bold'); doc.setFontSize(8); doc.setTextColor(29, 78, 216);
    doc.text('FACTURAS EMITIDAS', ml + 2, y + 4.2);
    doc.setTextColor(0, 0, 0);
    y += 7;

    const colsFac = [
      { label: 'FOLIO',  w: 30, align: 'left'  as const },
      { label: 'FECHA',  w: 28, align: 'left'  as const },
      { label: 'TOTAL',  w: 33, align: 'right' as const },
      { label: 'PAGADO', w: 33, align: 'right' as const },
      { label: 'SALDO',  w: 33, align: 'right' as const },
      { label: 'ESTADO', w: cw - 157, align: 'center' as const },
    ];
    drawTableHeader(colsFac);
    facsPend.forEach((f, i) => {
      const estado = getEstadoPagoFactura(f);
      const badge = BADGE_ESTADO[estado];
      drawTableRow([
        { text: f.numeroFactura,                                                w: 30 },
        { text: new Date(f.fecha).toLocaleDateString('es-MX'),                  w: 28 },
        { text: '$' + fmtPeso(f.total),                                         w: 33, align: 'right' },
        { text: '$' + fmtPeso(getMontoPagadoFactura(f)),                        w: 33, align: 'right' },
        { text: '$' + fmtPeso(getSaldoFactura(f)),                              w: 33, align: 'right', bold: true },
        { text: badge.label,                                                    w: cw - 157, align: 'center' },
      ], i);
    });
    y += 3;
  }
  ...
  if (trabsPend.length > 0) {
    doc.setFillColor(240, 253, 244); doc.rect(ml, y, cw, 6, 'F');
    doc.setFont('helvetica', 'bold'); doc.setFontSize(8); doc.setTextColor(21, 128, 61);
    doc.text('TRABAJOS SIN FACTURA', ml + 2, y + 4.2);
    doc.setTextColor(0, 0, 0);
    y += 7;

    const wDesc = 55, wVeh = 45, wFech = 25, wMon = 25, wSal = 25, wEst = cw - 175;
    drawTableHeader([
      { label: 'DESCRIPCIÓN', w: wDesc, align: 'left'  },
      { label: 'UNIDAD',      w: wVeh,  align: 'left'  },
      { label: 'FECHA',       w: wFech, align: 'left'  },
      { label: 'TOTAL',       w: wMon,  align: 'right' },
      { label: 'PAGADO',      w: wMon,  align: 'right' },
      { label: 'SALDO',       w: wSal,  align: 'right' },
      { label: 'ESTADO',      w: wEst,  align: 'center'},
    ]);
    trabsPend.forEach((t, i) => {
      const veh = vehiculos.find(v => v.id === t.vehiculoId);
      const vehLabel = veh ? `${veh.anio} ${veh.marca} ${veh.modelo}`.trim() : '—';
      const badge = BADGE_ESTADO[getEstadoPago(t)];
      drawTableRow([
        { text: t.descripcion,                                  w: wDesc },
        { text: vehLabel,                                       w: wVeh  },
        { text: new Date(t.fecha).toLocaleDateString('es-MX'), w: wFech },
        { text: '$' + fmtPeso(t.total),                        w: wMon,  align: 'right' },
        { text: '$' + fmtPeso(getMontoPagado(t)),              w: wMon,  align: 'right' },
        { text: '$' + fmtPeso(getSaldo(t)),                    w: wSal,  align: 'right', bold: true },
        { text: badge.label,                                    w: wEst,  align: 'center' },
      ], i);
    });
    y += 3;
  }
  ...
  if (totalPend > 0) {
    ...
    y += 2;
    doc.setFillColor(254, 242, 242); doc.rect(bx, y, bw, 14, 'F');
    doc.setDrawColor(254, 202, 202); doc.rect(bx, y, bw, 14, 'S');
    doc.setFont('helvetica', 'bold'); doc.setFontSize(7.5); doc.setTextColor(185, 28, 28);
    doc.text('TOTAL POR COBRAR', bx + 3, y + 5.5);
    doc.setFontSize(13); doc.setTextColor(153, 27, 27);
    doc.text('$' + fmtPeso(totalPend), bx + bw - 3, y + 11.5, { align: 'right' });
    doc.setTextColor(0, 0, 0);
    y += 18;
  }
  ...
  // ── Footer ──
  y += 4;
  doc.setDrawColor(203, 213, 225); doc.line(ml, y, ml + cw, y); y += 4;
  doc.setFont('helvetica', 'normal'); doc.setFontSize(7); doc.setTextColor(148, 163, 184);
  doc.text(`Generado: ${fechaHoy}  ·  MICRO DIESEL DE MÉRIDA`, ml, y);

  const filename = `CxC_${cliente.nombre.replace(/[^a-zA-Z0-9]/g, '_')}_${new Date().toISOString().split('T')[0]}`;
...
export function VistaCuentasPorPagar({
    ...
    (t.manoDeObraItems ?? [])
      ...
      .map(i => {
        ...
        return {
          item: i,
          trabajoId: t.id,
          trabajoDesc: t.descripcion,
          trabajoFecha: t.fecha,
          clienteNombre: cli?.nombre ?? '—',
          vehiculoLabel: veh ? `${veh.anio} ${veh.marca} ${veh.modelo}`.trim() : '—',
        };
      })
  );
  ...
  const provGroups = (() => {
    ...
    for (const ei of extItems) {
      ...
      const g = map.get(key)!;
      g.items.push(ei);
      g.costoTotal += ei.item.costoTaller ?? 0;
      g.saldoTotal += getSaldoServicio(ei.item);
    }
    return [...map.values()].sort((a, b) => b.saldoTotal - a.saldoTotal);
  })();

  const totalServiciosPendiente = provGroups.reduce((s, g) => s + g.saldoTotal, 0);

  const handlePagoServicio = (trabajoId: string, itemId: string, saldo: number) => {
    if (pagoServForm.monto <= 0) return;
    onPagarServicioExterno(trabajoId, itemId, {
      monto: Math.min(pagoServForm.monto, saldo),
      fecha: pagoServForm.fecha,
      metodoPago: pagoServForm.metodoPago,
      nota: pagoServForm.nota || undefined,
    });
    setPagoServForm({ monto: 0, fecha: hoy, metodoPago: 'Efectivo', nota: '' });
  ...
  const totalPagadoPorProveedor = (() => {
    ...
    for (const o of ordenesFiltradas) {
      map[o.proveedorId] = (map[o.proveedorId] ?? 0) + getMontoPagadoOrden(o);
    }
    return map;
  })();
  ...
  return (
    <div>
      ...
      <div className="flex gap-1 mb-6 bg-slate-100 rounded-xl p-1 w-fit">
        <button
          onClick={() => setTabPago('refacciones')}
          className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-semibold transition-all ${
            tabPago === 'refacciones' ? 'bg-white text-slate-800 shadow-sm' : 'text-slate-500 hover:text-slate-700'
          }`}
        >
          🏪 Refacciones
          {totalPendiente > 0 && (
            <span className="text-xs bg-rose-100 text-rose-600 font-bold px-1.5 py-0.5 rounded-full">
              {ordenesPagables.filter(o => getEstadoPagoOrden(o) !== 'pagado').length}
            </span>
          )}
        </button>
        <button
          ...
          className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-semibold transition-all ${
            tabPago === 'servicios' ? 'bg-white text-slate-800 shadow-sm' : 'text-slate-500 hover:text-slate-700'
          }`}
        >
          🏭 Servicios Externos
          {totalServiciosBadge > 0 && (
      ...
      {tabPago === 'servicios' && (
        <div>
          {extItems.length === 0 ? (
            <div className="text-center py-14 text-slate-400">
              <div className="text-5xl mb-3">🏭</div>
              <p className="font-medium text-slate-500">Sin servicios externos registrados</p>
              <p className="text-sm mt-1">
                Los servicios externos aparecen aquí cuando agregas un &quot;Servicio Externo&quot; en una orden de trabajo.
              </p>
            </div>
          ) : (
            <div>
              {totalServiciosPendiente > 0 && (
                <div className="bg-orange-50 border border-orange-200 rounded-xl px-5 py-4 mb-5 flex flex-wrap gap-6 items-center">
                  <div>
                    <div className="text-xs font-bold text-orange-600 uppercase tracking-widest mb-1">Total por Pagar — Servicios</div>
                    <div className="text-2xl font-extrabold text-orange-700">${fmt(totalServiciosPendiente)}</div>
                  </div>
                  <div>
                    <div className="text-xs text-orange-500 mb-1">Proveedores con saldo</div>
                    <div className="text-lg font-bold text-orange-600">{provGroups.filter(g => g.saldoTotal > 0).length}</div>
                  </div>
                </div>
              )}

              <div className="space-y-3">
                {provGroups.map(grupo => {
                  const isExpProv = expandidoProv === grupo.key;
                  const itemsPendientes = grupo.items.filter(ei => getEstadoServicio(ei.item) !== 'pagado');

                  return (
                    <div key={grupo.key} className="border border-slate-200 rounded-xl overflow-hidden">
                      {/* Provider header */}
                      <button
                        type="button"
                        onClick={() => setExpandidoProv(isExpProv ? null : grupo.key)}
                        className={`w-full grid grid-cols-1 sm:grid-cols-5 gap-2 items-center px-4 py-3 text-left transition-colors ${
                          isExpProv ? 'bg-orange-50' : 'bg-white hover:bg-orange-50/50'
                        }`}
                      >
                        <div className="sm:col-span-3">
                          <div className="flex items-center gap-2 flex-wrap">
                            ...
                            {itemsPendientes.length > 0 && (
                              <span className="text-xs bg-orange-100 text-orange-700 font-bold px-2 py-0.5 rounded-full">
                                {itemsPendientes.length} pendiente{itemsPendientes.length !== 1 ? 's' : ''}
                              </span>
                            )}
                            {itemsPendientes.length === 0 && (
                          ...
                          <div className="text-xs text-slate-400 mt-0.5">
                            {grupo.items.length} servicio{grupo.items.length !== 1 ? 's' : ''} registrado{grupo.items.length !== 1 ? 's' : ''}
                          </div>
                        </div>
                        <div className="text-right">
                          <div className="text-xs text-slate-400 uppercase tracking-wide">Costo total</div>
                          <div className="font-semibold text-slate-700">${fmt(grupo.costoTotal)}</div>
                        </div>
                        <div className="text-right">
                          <div className="text-xs text-slate-400 uppercase tracking-wide">Saldo</div>
                          <div className={`font-bold text-lg ${grupo.saldoTotal > 0 ? 'text-rose-600' : 'text-emerald-500'}`}>
                      ...
                      {/* Provider drill-down */}
                      {isExpProv && (
                        <div className="border-t border-slate-200 bg-orange-50/30 p-4 space-y-3">
                          {grupo.items
                            .sort((a, b) => b.trabajoFecha.localeCompare(a.trabajoFecha))
                            .map(ei => {
                              const saldo   = getSaldoServicio(ei.item);
                              const estado  = getEstadoServicio(ei.item);
                              const pagado  = getMontoPagadoServicio(ei.item);
                              const badge   = BADGE_ESTADO[estado];
                              const itemKey = `${ei.trabajoId}::${ei.item.id}`;
                              const isExpItem = expandidoItem === itemKey;

                              return (
                                <div key={itemKey} className="bg-white border border-slate-200 rounded-xl overflow-hidden">
                                  <div className={`grid grid-cols-1 sm:grid-cols-6 gap-2 items-center px-4 py-3 ${isExpItem ? 'bg-slate-50' : ''}`}>
                                    <div className="sm:col-span-3">
                                      <div className="flex items-center gap-2 flex-wrap">
                                        <span className="font-medium text-slate-800">{ei.item.concepto}</span>
                                        <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${badge.cls}`}>{badge.label}</span>
                                      </div>
                                      <div className="text-xs text-slate-400 mt-0.5 flex gap-2 flex-wrap">
                                        <span>{new Date(ei.trabajoFecha + 'T00:00:00').toLocaleDateString('es-MX')}</span>
                                        <span>· {ei.clienteNombre}</span>
                                        <span>· {ei.vehiculoLabel}</span>
                                        <span className="italic truncate max-w-[180px]">· {ei.trabajoDesc}</span>
                                      </div>
                                    </div>
                                    <div className="text-right">
                                      <div className="text-xs text-slate-400">Costo</div>
                                      <div className="font-semibold text-rose-600">${fmt(ei.item.costoTaller ?? 0)}</div>
                                    </div>
                                    <div className="text-right">
                                      <div className="text-xs text-slate-400">Pagado</div>
                                      <div className="font-semibold text-emerald-600">${fmt(pagado)}</div>
                                    </div>
                                    <div className="flex items-center justify-end gap-2">
                                      ...
                                      {estado !== 'pagado' && (
                                        <Btn
                                          size="sm"
                                          variant={isExpItem ? 'ghost' : 'success'}
                                          onClick={() => {
                                            setExpandidoItem(isExpItem ? null : itemKey);
                                            setPagoServForm({ monto: 0, fecha: hoy, metodoPago: 'Efectivo', nota: '' });
                                          }}
                                        >
                                          {isExpItem ? '✕' : '+ Pago'}
                                  ...
                                  {/* Payment history */}
                                  {(ei.item.pagosServicio ?? []).length > 0 && (
                                  ...
                                  {isExpItem && estado !== 'pagado' && (
                                    <div className="px-4 pb-4 pt-3 border-t border-slate-200 bg-slate-50">
                                      <p className="text-xs font-bold text-slate-500 uppercase tracking-widest mb-3">
                                        Registrar Pago — saldo: ${fmt(saldo)}
                                      </p>
                                      <div className="grid grid-cols-1 sm:grid-cols-5 gap-3">
                                        <div>
                                          <Label>Fecha</Label>
                                          <Input type="date" value={pagoServForm.fecha}
                                            onChange={e => setPagoServForm(f => ({ ...f, fecha: e.target.value }))} />
                                        </div>
                                        <div>
                                          <Label>Monto ($)</Label>
                                          <Input type="number" placeholder={fmt(saldo)} min="0.01" step="0.01"
                                            value={pagoServForm.monto || ''}
                                            onChange={e => setPagoServForm(f => ({ ...f, monto: Number(e.target.value) }))} />
                                        </div>
                                        <div>
                                          <Label>Método</Label>
                                          <Select value={pagoServForm.metodoPago}
                                            onChange={e => setPagoServForm(f => ({ ...f, metodoPago: e.target.value }))}>
                                            {['Efectivo','Transferencia','Tarjeta','Cheque','Otro'].map(m => <option key={m}>{m}</option>)}
                                          </Select>
                                        </div>
                                        <div>
                                          <Label>Nota (opcional)</Label>
                                          <Input type="text" placeholder="Referencia..." value={pagoServForm.nota}
                                            onChange={e => setPagoServForm(f => ({ ...f, nota: e.target.value }))} />
                                        </div>
                                        <div className="flex items-end">
                                          <Btn variant="success" fullWidth
                                            disabled={pagoServForm.monto <= 0}
                                            onClick={() => handlePagoServicio(ei.trabajoId, ei.item.id, saldo)}>
                                            ✓ Registrar
                                          </Btn>
                                        </div>
                                      </div>
                                  ...
                                  {estado === 'pagado' && (
                                    <div className="px-4 py-2 border-t border-slate-100 bg-emerald-50">
                                      <p className="text-xs text-emerald-600 font-semibold">✅ Servicio completamente pagado.</p>
                                    </div>
                                  )}
                                </div>
                              );
      ...
      {/* ════════════════════════════════════════════════════════
          TAB: REFACCIONES (existing PO-based AP)
      ════════════════════════════════════════════════════════ */}
      {tabPago === 'refacciones' && (
        <div>
          ...
          {ordenesPagables.length > 0 && <>
            ...
            {proveedores.length > 0 && (
              <div className="mb-4">
                <Select value={filtroProveedorId} onChange={e => setFiltroProveedorId(e.target.value)}>
                  ...
                  {proveedores.filter(p => ordenesPagables.some(o => o.proveedorId === p.id))
            ...
            {ordenesFiltradas.length > 0 && Object.entries(totalPagadoPorProveedor).filter(([, v]) => v > 0).length > 0 && (
              <div className="mt-5 space-y-2">
                ...
                {Object.entries(totalPagadoPorProveedor).filter(([, v]) => v > 0).map(([provId, total]) => {
                  ...
                  return (
                    <div key={provId} className="flex items-center justify-between bg-emerald-50 border border-emerald-200 rounded-lg px-4 py-2 text-sm">
                      ...
                      <span className="font-bold text-emerald-700">Total pagado: ${total.toLocaleString('es-MX', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                    </div>
                  );
                })}
              </div>
