...
function loadHistory(): CotizacionGuardada[] {
  ...
  try {
    return JSON.parse(localStorage.getItem(COT_HISTORY_KEY) ?? '[]') as CotizacionGuardada[];
  } catch {
    return [];
  }
}

function saveToHistory(entry: CotizacionGuardada): void {
...
interface FormCotizacion {
  numeroCotizacion: string;   // empty until saved; assigned by assignNextNumber()
  clienteId: string;
  cliente: string;
  vehiculoId: string;
  marca: string;
  ...
  incluirIVA: boolean;
  inventario: string;         // Ayuntamiento — OBLIGATORIO
  ordenServicio: string;      // Ayuntamiento
  departamento: string;       // Ayuntamiento — dropdown
...
async function generarYDescargarPDF(plantilla: Plantilla, form: FormCotizacion) {
  ...
  const clienteNombre = plantilla === 'ayuntamiento' ? 'Ayuntamiento de Mérida' : plantilla === 'red_ambiental' ? 'Red Ambiental' : form.cliente;
  ...
  if (form.trabajo) {
    ...
    doc.setFont('helvetica','normal'); doc.setFontSize(8);
    const tLines = doc.splitTextToSize(form.trabajo, cw);
    doc.text(tLines, ml, y); y += tLines.length * 4 + 2;
  }
  y += 3;
  ...
  // Totals
  const tx=ml+cw-65, tw=65, nr=form.incluirIVA?4:3;
  doc.setDrawColor(180,190,210); doc.rect(tx,y,tw,6*nr+2,'S');
  const totRow=(label:string,value:string,bold=false,bg?:[number,number,number])=>{
    ...
    doc.text(label,tx+2,y+4.2); doc.text(value,tx+tw-2,y+4.2,{align:'right'}); doc.setTextColor(0,0,0);
  ...
  doc.text('TOTAL:',tx+2,y+4.3); doc.text('$'+fmtPeso(total),tx+tw-2,y+4.3,{align:'right'});
  ...
  if(form.observaciones){
    ...
    doc.setFont('helvetica','normal'); doc.setFontSize(8);
    const obsL=doc.splitTextToSize(form.observaciones,cw); doc.text(obsL,ml,y); y+=obsL.length*4+3;
  }

  // Signature
  y+=10; doc.setDrawColor(100,100,100); doc.line(ml,y,ml+60,y);
  doc.setFont('helvetica','normal'); doc.setFontSize(7); doc.text('Autorizado por',ml+15,y+4);
  ...
  return (
    <div className="mb-5">
      ...
      <div className="border border-slate-200 rounded-b-xl overflow-hidden">
        ...
        {items.length===0&&<div className="px-4 py-4 text-center text-slate-400 text-sm italic">Sin partidas — haz clic en &quot;+ Agregar&quot;</div>}
        {items.map((item,idx)=>(
...
function VistaPreviaContenido({ plantilla, form, onEditar, onNueva }:{
  plantilla:Plantilla; form:FormCotizacion; onEditar:()=>void; onNueva:()=>void;
}) {
  ...
  return (
    <div>
      ...
      <div className="flex gap-3 mb-6 flex-wrap">
        ...
        <Btn variant="ghost" onClick={onEditar}>✏️ Editar</Btn>
...
function FilaTotal({ label, value, bold=false, highlight }:{ label:string; value:number; bold?:boolean; highlight?:'blue'|'amber' }) {
  const cls = highlight==='blue' ? 'bg-blue-700 text-white px-3 py-2 rounded-lg' : highlight==='amber' ? 'bg-amber-50 px-3 py-1 rounded' : 'px-1 py-0.5';
  return (
...
function HistorialCotizaciones({ history, onVer }:{
  ...
  const filtradas = filtroCliente
    ? history.filter(h => h.cliente === filtroCliente)
    : history;
    ...
    <div className="mt-8">
      ...
      {filtradas.length === 0 ? (
        <div className="text-center py-10 text-slate-400">
          ...
          <p className="text-sm">
            {filtroCliente ? `Sin cotizaciones para "${filtroCliente}"` : 'Aún no hay cotizaciones guardadas'}
          </p>
        </div>
      ) : (
        <div className="space-y-2">
          ...
          {filtradas.map(entry => (
            <div key={entry.id} className="grid grid-cols-12 gap-2 items-center px-4 py-3 bg-white rounded-xl border border-slate-200 hover:border-indigo-200 hover:shadow-sm transition-all">
              <div className="col-span-2">
              ...
              <div className="col-span-4">
                ...
                <p className="text-xs text-slate-400">
                  {entry.plantilla==='ayuntamiento'?'🏛️':entry.plantilla==='red_ambiental'?'♻️':'🔧'} {PLANTILLAS.find(p=>p.key===entry.plantilla)?.label}
                </p>
              </div>
...
export function VistaCotizaciones({
  ...
  // Vehicles for the selected client
  ...
  // ── Save: assign number + persist + go to preview ─────────────────────────
  const handleGuardar = () => {
  ...
  // ══════════════════════════════════════════════════════════════════════════
  // Screen: INICIO (selector + history)
  // ══════════════════════════════════════════════════════════════════════════
  if (pantalla === 'inicio') {
    return (
      <div>
        ...
        <HistorialCotizaciones history={history} onVer={handleVerEntry}/>
      </div>
    );
  }
    ...
    return (
      <div>
        <div className="flex items-center gap-3 mb-6">
          <button onClick={()=>setPantalla('inicio')} className="text-slate-400 hover:text-slate-700 transition-colors text-lg">←</button>
          <SectionTitle title={`Cotización — ${labelPlantilla}`} subtitle="Completa los datos y guarda para asignar número"/>
        </div>
        ...
        <div className="flex items-center gap-3 mb-5 px-4 py-3 bg-amber-50 border border-amber-200 rounded-xl">
          ...
          <p className="text-sm text-amber-700">
            El número de cotización (<strong>COT-XXX</strong>) se asignará automáticamente al guardar.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-5">
          ...
          {/* Red Ambiental: fixed provider number */}
        ...
        <div className="flex gap-3 flex-wrap items-center">
          <Btn variant="ghost" onClick={()=>setPantalla('inicio')}>← Cancelar</Btn>
